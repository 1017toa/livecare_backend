...
class LangChainHandler:
    def __init__(self):
        ...
        self.gpt4o_model = ChatOpenAI(model="gpt-4o-mini")
    ...
    async def extract_metadata(self, text, pill_info, temperature=0.0):
        ...
        chain = prompt | self.gpt4o_model | JsonOutputParser()
    ...
    async def create_medical_chart(self, text, temperature=0.0):
        ...
        prompt = ChatPromptTemplate.from_template(base_prompt)
        chain = prompt | self.gpt4o_model | StrOutputParser()
    ...
    async def create_multidisciplinary_care(self, patient_info, drug_info, temperature=0.0):
        ...
        base_prompt = self.load_prompt("create_multidisciplinary_care_0.0.0")
        prompt = ChatPromptTemplate.from_template(base_prompt)
        chain = prompt | self.gpt4o_model | StrOutputParser()
